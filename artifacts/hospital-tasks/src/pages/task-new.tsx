      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">
            Classifique a demanda para a triagem.
          </p>
        </div>
      </div>
              ...
              render={({ field }) => (
                <FormItem className="space-y-3">
                  <FormLabel className="text-base font-bold">
                    Prioridade (Classificação de Triagem)
                  </FormLabel>
                  <FormControl>
                    <RadioGroup
                    ...
                    >
                      {(Object.keys(PRIORITY_LABELS) as TaskInputPriority[]).map((prio) => (
                        <FormItem key={prio} className="flex-1">
                          ...
                          <FormLabel
                            className={`flex flex-col items-center justify-center rounded-xl border-2 p-3 md:p-4 hover:bg-accent hover:text-accent-foreground cursor-pointer transition-all ${
                              field.value === prio
                                ? `${PRIORITY_COLORS[prio].bg} ${PRIORITY_COLORS[prio].text} ${PRIORITY_COLORS[prio].border}`
                                : "border-muted bg-popover"
                            }`}
                          >
                            <div
                              className={`w-5 h-5 rounded-full mb-2 shadow-sm ${
                                prio === "critical"
                                  ? "bg-red-500"
                                  : prio === "high"
                                  ? "bg-orange-500"
